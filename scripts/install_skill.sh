...
destination_root="$HOME/.gemini/skills"
...
mkdir -p "$destination"
cp -a "$skill_dir/SKILL.md" "$skill_dir/assets" \
  "$skill_dir/references" "$skill_dir/scripts" "$destination/"
chmod +x "$destination"/scripts/*.sh "$destination"/scripts/*.py
printf 'Installed Gemini CLI Skill at %s\nRun /skills reload, then ask Gemini CLI to use recamera-rknn-dev.\n' "$destination"
